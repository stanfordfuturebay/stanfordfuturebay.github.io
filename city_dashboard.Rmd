...
colors <- primary.colors(nrow(variables)+1, 5, no.white = T) %>% .[-1]
...
**Correlation Plot (Tab 1)** - Each dot is a block group. Keep in mind that correlation is not causation. Select two variables to plot by:
...
**Longitudinal Plot (Tab 2)** - The primary longitudinal data available is on evictions. Census data can only be plotted as the change from 2009-2013 summary data to 2014-2018 summary data. The average line is shown in orange, and individual block groups can be added in blue
...
**About the data** - Most data is from the American Communities Survey 2009-2013 or 2014-2018 5-year Detailed Tables (shown by default in plots, map, and table):
...
- % Households Earning < \$25,000: B19001, HOUSEHOLD INCOME IN THE PAST 12 MONTHS (IN 2018 INFLATION-ADJUSTED DOLLARS)
...
- % No Broadband: B28008, PRESENCE OF A COMPUTER AND TYPE OF INTERNET SUBSCRIPTION IN HOUSEHOLD (only available for 2014-2018)
...
observeEvent(input$V4,{
  ...
  leafletProxy('map') %>% 
    ...
    showGroup(
      variables %>% 
        filter(label == input$V4) %>% 
        pull(short)
    )
  
  for(x in 1:nrow(variables)){
    
    if(variables[x,2] %in% isolate(input$V4)){
