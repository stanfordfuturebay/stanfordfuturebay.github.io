...
Join Shelter and Place Data with Essential Business Data
```{r}
...
block_group_data <- sj_socialdistancing %>% 
  ...
  mutate(`% Completely at Home` = (completely_home_device_count/device_count*100) %>% round(1), 
         `% not completely at home` = (100 - `% Completely at Home`)) %>%  right_join(essential_workers_method1, by = c("origin_census_block_group" = "h_bg")) %>% 
  right_join(essential_workers_method2, by = c("origin_census_block_group" = "h_bg"))

block_group_data
```
Visualize Predictive Power of Each Method Data
Method 1
```{r}
block_group_data %>%
  ggplot(aes(
