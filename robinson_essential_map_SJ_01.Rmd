...
Visualize the number of workers in each 2-digit NAICS code
```{r}

visuzlize_4_dig_naics <- CA_essential %>% 
  mutate(NAICS_4_dig = str_sub(NAICS, 1,4)) %>%
  group_by(NAICS_4_dig, `CA Essential`) %>% 
  summarize(count = n()) %>% 
  ungroup() %>% 
  ...
  mutate(EmpS = replace_na(EmpS,0)) %>% # assume if not listed it is not essential, and if no employees listed that the number is zero
  mutate(fracEssential_4dig =replace(fracEssential_4dig, NAICS_4_dig== "8141", 0), EmpS = replace(EmpS, NAICS_4_dig == "8141",0)) %>%   #Manually assign NAICS code 8141 as nonessential 
  mutate(essEmpS = EmpS*fracEssential_4dig) %>% 
  arrange(desc(essEmpS)) %>%
    slice(1:30) %>% 
    ggplot(aes(x = reorder(NAICS_4_dig, -essEmpS), y = essEmpS))+
  geom_bar(stat = "identity")+
  ggtitle(" Number of Essential Workers by 4digit NAICS code")+
...
visuzlize_4_dig_naics

visualize_2_dig_naics <- ggplot(data = CA_essential_weighted_grouped_ca1, aes(x = reorder(LODES_variable, -EmpS), y = EmpS))+
  geom_bar(stat = "identity")+
  ggtitle(" Number of Workers by 2-digit NAICS code")+
  theme_fivethirtyeight()+
  theme(
    plot.title = element_text(size=14, face="bold.italic"),
    axis.text.x =  element_text(angle = 90, size=14),
...
#sj_rac_essential_ca1 <- sj_rac_essential_ca1 %>% rename("fracMethod1" = fracEssential_2dig)
#saveRDS(select(sj_rac_essential_ca1, h_bg, fracMethod1), file = "/Users/spencerrobinson/Documents/covid19/Spencer_Robinson/essential_workers_method1.rds")
...
```{r}
essential_workers_visualize <- ggplot(data = sj_rac_essential_ca1, aes(x = fracEssential_2dig))+
  geom_histogram()+
  labs(y = "Number of Block Groups", x = "% Essential Workers")+
  ggtitle("Distribution of Block Group Essential Worker Data Method 1")

essential_workers_visualize
```

Visualization of Essential Workers by Industry
```{r}
...
Visualize Essential v Non-Essential 2-digit NAICS codes
```{r}
visualize_essential_workers <- ggplot(data = CA_essential_grouped, aes(x = LODES_variable, y = fracEssential, fill = binaryEssential))+
  geom_bar(stat = "identity")+
  ggtitle(" Essential Workers by 2-digit NAICS code")+
  theme_fivethirtyeight()+
  theme(
    plot.title = element_text(size=14, face="bold.italic"),
    axis.text.x =  element_text(angle = 90, size=14),
...
#sj_rac_essential_grouped <- sj_rac_essential_grouped %>% rename("fracMethod2" = fracEssential )
#saveRDS(select(sj_rac_essential_grouped, h_bg, fracMethod2), file = "/Users/spencerrobinson/Documents/covid19/Spencer_Robinson/essential_workers_method2.rds")

essential_workers_visualize <- ggplot(data = sj_rac_essential_grouped, aes(x = fracEssential))+
  geom_histogram()+
  labs(y = "Number of Block Groups", x = "% Essential Workers")+
  ggtitle("Distribution of Block Group Essential Worker Data Method 2 ")
